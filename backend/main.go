...
func main() {
	// 1. Cargar variables globales (soporte para .env o variables del sistema)
	config.LoadConfig()

	// 2. Inicializar conexión a base de datos PostgreSQL y sembrar configuración inicial
	...
	// 3. Inicializar repositorios y cliente SSH
	sshClient := sshclient.NewSSHClient()
	atrecRepo := database.NewMemoryATRECRepository()

	// 4. Inicializar handlers HTTP con soporte para múltiples servidores (inyectando dbRepo)
	...
	// 5. Configurar enrutador HTTP puro para API REST
	mux := http.NewServeMux()

	// Handler para CRUD de configuraciones (PostgreSQL con fallback seguro en memoria)
	mux.HandleFunc("/api/v1/configs", configHandler.ConfigsRouter)
	mux.HandleFunc("/api/v1/configs/", configHandler.ConfigsRouter)
	mux.HandleFunc("/api/configs", configHandler.ConfigsRouter)
	mux.HandleFunc("/api/configs/", configHandler.ConfigsRouter)

	// Endpoint raíz informativo JSON
	...
	// Endpoints de logs con Filtros y Paginación (Next.js compatible, soportan ?server_id=X)
	...
	// Endpoints para listar archivos y leer archivos individuales (soportan ?server_id=X)
	...
	// ==============================================================
	// Endpoints V1: Módulo de Métricas de Hardware (KPIs, soportan ?server_id=X)
	// ==============================================================
	mux.HandleFunc("/api/v1/hardware/tube-health", hardwareHandler.TubeHealthHandler)
	mux.HandleFunc("/api/v1/hardware/tube-warmup", hardwareHandler.TubeWarmupHandler)
	mux.HandleFunc("/api/v1/hardware/warmup-routines", hardwareHandler.TubeWarmupHandler)
	mux.HandleFunc("/api/hardware/tube-warmup", hardwareHandler.TubeWarmupHandler)
	...
	// ==============================================================
	// Endpoints V1: Módulo de Alertas por Subsistema TSM (soportan ?server_id=X)
	// ==============================================================
	...
	// ==============================================================
	// Endpoints V1: Módulo de Telemetría Específica (soportan ?server_id=X)
	// ==============================================================
	...
	log.Printf(" API Tube Warm-up:     http://localhost%s/api/v1/hardware/tube-warmup?server_id=1", config.ServerPort)
