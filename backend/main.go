...
func main() {
	...
	// Configurar repositorio ATREC con persistencia PostgreSQL (o fallback en memoria si BD no está disponible)
	var atrecRepo database.ATRECRepository
	if dbRepo != nil && dbRepo.GetDB() != nil {
		atrecRepo = database.NewPostgresATRECRepository(dbRepo.GetDB())
		log.Println("[ATREC] Repositorio ATREC persistente en PostgreSQL activo")
	} else {
		atrecRepo = database.NewMemoryATRECRepository()
		log.Println("[ATREC WARNING] Usando repositorio ATREC en memoria (fallback sin PostgreSQL)")
	}
